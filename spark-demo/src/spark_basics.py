...
df_people_filtered = df_people.withColumn('net_worth',
                                          regexp_extract(col('net_worth'), '^\$(\\d+).*$', 1).cast('int')).filter(
    col('net_worth') > 60)
...
df_last = df_prelast.filter(col('age').isNotNull()).select(col('rank'), col('name'), col('net_worth'), col('bday'),
...
df_last.write.option("header", 'true').mode('overwrite').csv("data/1.csv")
